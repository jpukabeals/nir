...
# orei-manure | detect missing data ---------------------------------------
...
# changing data from 'as is' to dry matter basis
dat1 <- dat %>% 
  rename_all(.,tolower) %>% 
  filter(`product name`=="Hay") %>%
  mutate(datetime=`date/time of analysis`,
...
# https://fyi.extension.wisc.edu/forage/relative-forage-quality-rfq-indexing-legumes-and-grasses-for-forage-quality/
