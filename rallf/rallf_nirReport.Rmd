...
It's weird the RFQ and RFV do not correlate closely. Is this because of the weird NDFD values?


### Expected results

Alfalfa varieties (HarvXtra) will have lower lignin content and higher NDFD than conventional varieties. 


### Discussion topics

What forage quality comparisons are we making? If we are only making comparisons among alfalfa cuttings, let's use RFV not RFQ. RFQ relies on NDFD, which is much harder to predict with NIRS. There is lot of variability in NDFD in wet chemsity due to variability in rumen fluid composition from donor cows, this variability extends into the NIRS database for NDFD, which reduces the accuracy of the NIRS predictions. RFQ is useful when comparing between mixtures of legumes and grasses, but since we are only making comparisons between alfalfa, RFV allows us to rely on just ADF and NDF, which NIRS performs well at predicting. 

What equations are appropiate? Our generalist hay equation is 
